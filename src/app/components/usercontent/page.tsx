...
function UserContent() {
  ...
  const {isLogin, setIsLogin, currIndex,setCurrIndex,setUserName,email,setEmail,chatRooms,setChatRooms,setContent,content,chatRoomData,setChatRoomData,currRid,newContent,setNewContent,quickPrompt,setQuickPrompt} = context || {};
  const [prompt, setPrompt] = React.useState('')
  const [currPrompt, setCurrPrompt] = React.useState('')
...
// React.useEffect(=>{
// if(quickPrompt!=='')
// setCurrPrompt(quickPrompt)
// },[quickPrompt])

  const runPrompt = async () => {
 
    // if(prompt === '' && quickPrompt !== undefined){
    ...
    result = await Content(quickPrompt,currRid, loggedIn, email,"gen_data");
    ...
    if (result) {
      ...
      setChatRooms?.((prevChatRooms: any) => {
        ...
        updatedChatRooms[currIndex!] = {prmt:quickPrompt,id:updatedChatRooms[currIndex!].id};
        updatedChatRooms[currIndex!] = {prmt:quickPrompt,id:currRid};
...
React.useEffect(()=>{
console.log("From- user content-currIndex",currIndex, "chatRoomData",chatRoomData, "newContent",newContent)
        ...
        <div className={`w-full ${sty1}`}>
          ...
          <div className={`w-4/5 lg:w-3/5 bg-white ${sty1} rounded-2xl p-2`}>
            <input 
              ...
              onChange={(e) => {setQuickPrompt?.(e.target.value),setPrompt(e.target.value);}}
              value={quickPrompt} 
              disabled={isLoading} 
            />
            <div 
              className={` right-0 mr-1 size-9 md:size-10d rounded-full flex flex-row justify-center items-center bg-gradient-to-r from-fuchsia-500 via-violet-600 to-blue-500 p-1   ml-1  ${quickPrompt === '' || isLoading ?'cursor-not-allowed':'cursor-pointer'}`} 
              onClick={!isLoading && quickPrompt !== '' ? () => {setPrompt(quickPrompt); runPrompt();} : undefined} 
             >
              <Image
