...
function FileUpload ()  {
    ...
    const {isLogin, setIsLogin, setUserName,email, files,setFiles,setShowError} = context || {};
    ...
    const sendFile =  async () => {
        ...
        if(files.length>0){
         ...
         if(getFileResponse.status===200)
         {
            ...
            setShowError?.("Error on getting the response, please try after some time")
         }
        }
    }
    ...
    return (
        <div className={`${sty2}`}>
            ...
            <ul className ='mt-2 max-h-40 overflow-y-auto'>
                ...
                {
                    isUploadDisabled && (
                        <span className='bg-pink-100 text-red-800 rounded-md p-2 mt-2'>!!Files must have same format !</span>
                    )
                }
